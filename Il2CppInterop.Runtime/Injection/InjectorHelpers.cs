...
internal static unsafe class InjectorHelpers
{
    ...
    private static ProcessModule? CachedIl2CppModule;

    internal static ProcessModule Il2CppModule
    {
        get
        {
            if (CachedIl2CppModule is not null)
                return CachedIl2CppModule;

            var name = Il2CppInteropRuntime.GameAssemblyFullName;
            return CachedIl2CppModule = Process.GetCurrentProcess()
                .Modules.OfType<ProcessModule>()
                .Single(x => x.ModuleName.Equals(name));
        }
    }
    ...
    {
        return NativeLibrary.TryGetExport(Il2CppInteropRuntime.Instance.Il2CppHandle, name, out address);
    }
